...
async function handleLogin(e) {
    ...
    try {
        ...
        // 登录成功后立即发送心跳（内部会先删除匿名记录，再添加注册记录）
        await sendOnlineHeartbeat();
        // 心跳完成后才更新在线人数，避免显示中间状态
        updateOnlineCount();
...
async function handleRegister(e) {
    ...
    try {
        ...
        // 注册成功后立即发送心跳（内部会先删除匿名记录，再添加注册记录）
        await sendOnlineHeartbeat();
        // 心跳完成后才更新在线人数，避免显示中间状态
        updateOnlineCount();
...
// 在线用户追踪
const ONLINE_USERS_KEY = 'class_suggestions_online_users';
const ONLINE_HEARTBEAT_INTERVAL = 15000; // 15秒心跳
const ONLINE_TIMEOUT = 45000; // 45秒超时视为离线（增加容错）
const REGISTERED_COUNT_CACHE_KEY = 'class_suggestions_registered_count_cache';
...
async function loadUserStats() {
    // 立即从缓存加载
    loadCachedRegisteredCount();
    
    // 异步更新，不阻塞页面
    loadRegisteredCount().catch(() => {});
    updateOnlineCount().catch(() => {});
    
    // 设置 Realtime 订阅
    setupOnlineUsersRealtime();
}

// 加载缓存的注册用户数
function loadCachedRegisteredCount() {
    try {
        const cached = localStorage.getItem(REGISTERED_COUNT_CACHE_KEY);
        if (cached) {
...
function cacheRegisteredCount(count) {
    try {
        localStorage.setItem(REGISTERED_COUNT_CACHE_KEY, JSON.stringify({
            count,
            timestamp: Date.now()
        }));
    } catch (e) {
...
async function loadRegisteredCount() {
    ...
    try {
        ...
        const registeredCount = document.getElementById('registeredCount');
        if (registeredCount) {
            registeredCount.textContent = count || 0;
        }
        
        // 缓存结果
        cacheRegisteredCount(count || 0);
    } catch (error) {
        console.error('获取注册用户数失败:', error);
    }
}
...
function setupOnlineTracking() {
    ...
    console.log('[在线追踪] 初始化在线状态追踪...');
    
    // 立即发送心跳
    sendOnlineHeartbeat();
    
    // 定期发送心跳
    const heartbeatTimer = setInterval(sendOnlineHeartbeat, ONLINE_HEARTBEAT_INTERVAL);
    
    // 页面关闭时标记离线
    window.addEventListener('beforeunload', markUserOffline);
    window.addEventListener('unload', markUserOffline);
    
    // 页面可见性变化时
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            console.log('[在线追踪] 页面重新可见，发送心跳并更新在线人数');
            sendOnlineHeartbeat();
            updateOnlineCount();
        }
    });
...
// 发送在线心跳（使用固定的session ID，避免登录/退出时创建多条记录）
async function sendOnlineHeartbeat() {
    if (!supabaseClient) {
        console.warn('[在线追踪] Supabase 未初始化');
        return;
    }

    const isAdmin = currentUser && currentUser.isAdmin;

    console.log(`[在线追踪] 发送心跳 - Session: ${anonymousUserId}, 用户: ${currentUser?.username || '匿名'}, 管理员: ${isAdmin}`);

    try {
        // 始终使用 anonymousUserId 作为 session key，登录/退出时只更新记录信息
        const userInfo = {
            user_id: anonymousUserId,  // 固定使用 session ID
            is_registered: !!currentUser,
            username: currentUser ? currentUser.displayName : '匿名用户',
            is_admin: isAdmin,
        ...
        if (error) {
            console.error('[在线追踪] 更新在线状态失败:', error);
        } else {
            console.log('[在线追踪] 心跳发送成功:', data);
        }
    } catch (error) {
        console.error('[在线追踪] 心跳发送异常:', error);
    }
}

// 标记用户离线（使用固定的session ID）
async function markUserOffline() {
    if (!supabaseClient) return;

    console.log(`[在线追踪] 标记用户离线: ${anonymousUserId}`);

    try {
        // 使用固定的 session ID 删除记录
        await supabaseClient
            .from('online_users')
            .delete()
            .eq('user_id', anonymousUserId);
    } catch (error) {
        console.error('[在线追踪] 标记离线失败:', error);
    }
}
...
async function updateOnlineCount() {
    ...
    try {
        const cutoffTime = new Date(Date.now() - ONLINE_TIMEOUT).toISOString();

        // 单次查询获取所有在线用户信息，避免多次查询导致的延迟
        const { data, error } = await supabaseClient
            .from('online_users')
            .select('is_admin')
        ...
        const adminCount = data ? data.filter(u => u.is_admin).length : 0;

        // 更新显示
        const onlineCount = document.getElementById('onlineCount');

        if (onlineCount) {
            onlineCount.textContent = totalCount || 0;

            // 如果有管理员在线，数字显示金色
            if (adminCount > 0) {
                onlineCount.style.color = '#ffd700';
            } else {
                onlineCount.style.color = '';
            }
        }

        console.log(`[在线追踪] 更新完成 - 总在线: ${totalCount}, 管理员: ${adminCount}`);
    } catch (error) {
        console.error('[在线追踪] 获取在线人数失败:', error);
    }
}

// 设置 Realtime 订阅，实时更新在线人数
function setupOnlineUsersRealtime() {
    if (!supabaseClient) {
        console.warn('[在线追踪] Supabase 未初始化，无法设置 Realtime');
        return;
    }
    
    console.log('[在线追踪] 设置 Realtime 订阅...');
    
    onlineUsersChannel = supabaseClient
        ...
        .on(
            ...
            (payload) => {
                console.log('[在线追踪] 收到实时更新:', payload);
                // 收到更新后立即重新查询
                updateOnlineCount();
            }
        )
        .subscribe((status) => {
            console.log('[在线追踪] Realtime 订阅状态:', status);
            if (status === 'SUBSCRIBED') {
